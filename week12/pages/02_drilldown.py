...
# initialise once, then keep alive
...
if st.session_state.sel_hood not in hoods_avail:          # guard: filters may have removed choice
...
k2.metric('Median Price', f"£{hood_df['price'].median():.0f}/night",
          f"£{hood_df['price'].median()-filtered['price'].median():+.0f} vs filtered market")
...
# BBD HIGHLIGHT colour type: blue for the chosen neighbourhood, grey for everything else
